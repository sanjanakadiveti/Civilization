...
public class GameController {
     ...
     *
     * Transitions the controller states based on clicked Tiles Updates menus
     * based on the new state Calls the updateTileView method to handle
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        // reset older tiles, if needed (updateTileView should handle all
        // colors)
        if (previousLastClicked != null) {
            previousLastClicked.updateTileView();
        }
    ...
    /**
     * Attempts to move the previously clicked tile to the most recently clicked
     * tile.
     *
     * @param newTile
    ...
    /**
     * Internal helper method to handle moving mechanics Uses the TerrainTile
     * form so that the AI can use it too
     */
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (!(end.isEmpty() && GridFX.adjacent(end, start)
                && ((Unit) start.getOccupant())
                .canMove(end.getType().getCost()))) {
            if (state == GameState.MOVING) {
                if (!end.isEmpty()) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText("Oh no, this tile is taken!"
                        + " Try an empty one.");
                    newAlert.setTitle("Not Allowed!");
                    ...
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText("Oh no, this tile is too far away!"
                        + " Try an adjacent one.");
                    newAlert.setTitle("Not Allowed!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    ...
                    newAlert.showAndWait();
                    // System.exit(0);
                } else if (!(((Unit) start.getOccupant())
                    .canMove(end.getType().getCost()))) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText("You don't have the endurance"
                        + " to get to this tile!");
                    newAlert.setTitle("Cannot Move!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    MediaPlayer player = new MediaPlayer(song);
                    player.play();
    ...
    /**
     * Internal helper to handle attacking mechanics Uses the TerrainTile form
     * so that the AI can use it too
     *
     * @param attacker
     * @param enemy
     */
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty() || enemy.getOccupant()
            .getOwner() == attacker.getOccupant().getOwner()
                || !((MilitaryUnit) attacker.getOccupant())
                .getCanAttack() || !GridFX.adjacent(attacker, enemy)) {
            if (state == GameState.ATTACKING) {
                if (enemy.isEmpty()) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText(
                        "The tile you tried to attack was empty!");
                    newAlert.setTitle("Cannot Attack!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    MediaPlayer player = new MediaPlayer(song);
                    player.play();
                    newAlert.showAndWait();
                    // System.exit(0);
                } else if (enemy.getOccupant()
                    .getOwner() == attacker.getOccupant().getOwner()) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText(
                        "You tried to attack yourself! Say NO to self-harm.");
                    newAlert.setTitle("Cannot Attack!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    MediaPlayer player = new MediaPlayer(song);
                    ...
                    newAlert.showAndWait();
                    // System.exit(0);
                } else if (!((MilitaryUnit) attacker.getOccupant())
                    .getCanAttack()) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText(
                        "You can't attack this tile on this turn."
                        + " Try again next time!");
                    newAlert.setTitle("Cannot Attack!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    ...
                    newAlert.showAndWait();
                    // System.exit(0);
                } else if (!GridFX.adjacent(attacker, enemy)) {
                    Alert newAlert = new Alert(Alert.AlertType.ERROR);
                    newAlert.setHeaderText(
                        "You can only attack an adjacent tile.");
                    newAlert.setTitle("Cannot Attack!");
                    String path = "src/main/java/view/sounds/beep.mp3";
                    Media song = new Media(new File(path).toURI().toString());
                    MediaPlayer player = new MediaPlayer(song);
        ...
        String path = "src/main/java/view/sounds/attack.mp3";
        Media song = new Media(new File(path).toURI().toString());
        MediaPlayer player = new MediaPlayer(song);
        player.play();
        //System.out.println("i played the sound mommy");
     ...
     *
     * @param bandits
     *            Bandit civ to consider the enemy
     */
    ...
    /**
     * Returns whether the current tile is a settlement, used when checking if
     * the recruit menu should be shown
     */
        ...
        for (int r = row - 1; r < row + 2; r++) {
            for (int c = col - 1; c < col + 2; c++) {
                if (c >= 0 && c < 10 && r >= 0 && r < 10) {
                    ...
                    if (occupant != null && occupant instanceof Settlement
                        && occupant.isFriendly()) {
                        return true;
                    }
                }
    ...
    /**
     * called when turn ends, collects all resources upon ending a turn from all
     * the map objects on the map and updates the civlization's resources
     */
    ...
    /**
     * Please forgive me for what I have done... - Marc Uses BFS to find the
     * nearest non-bandit unit Moves closer and attempts to attack Bandits are
     * dumb and will swarm you
     */
    public static void ai() {
        ...
        for (int r = 0; r < 10; r++) {
            for (int c = 0; c < 10; c++) {
                ...
                if (!tile.isEmpty() && !tile.getOccupant().isFriendly()) {
        ...
        if (banditHideout != null) {
            if (rand.nextInt(100) < 10) {
                TerrainTile spawn = getRandomAdjacentTile(banditHideout
                    .getRow(), banditHideout.getCol());
                if (spawn.isEmpty()) {
        ...
        for (TerrainTile tile : bandits) {
            ...
            int c = 0;
            if (chance < 25) {
                TerrainTile nearest = GridFX.getMap()
                    .getNearestNonBandit(tile.getRow(), tile.getCol());
                if (nearest != null) {
                    if (GridFX.adjacent(nearest, tile)) {
                        boolean wasSettlement = nearest
            ...
            if (newRow >= 0 && newRow < 10 && newCol >= 0 && newCol < 10) {
                ...
                // counter attack, and then the ai attempts to move the same
                // unit
